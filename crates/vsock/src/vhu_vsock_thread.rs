...
/// For example, if the thread is registered for the Rx queue, this event will be 
/// triggered when new EPOLLOUT events are pending on the epoll fd registered for
/// EPOLLOUT events.
pub(crate) const THREAD_SPECIFIC_BACKEND_EVENT: u16 = 3;
...
pub(crate) struct VhostUserVsockTxThread {
    ...
    vring_worker: Option<Arc<VringEpollHandler<ArcVhostBknd, VringRwLock, ()>>>,
    /// epoll fd to which new host connections are added to listen for EPOLLOUT events.
    epoll_file: File,
...
impl VhostUserVsockTxThread {
    ...
    pub fn new(epoll_out_fd: i32, thread_backend: Arc<VsockThreadBackend>) -> Result<Self> {
        // SAFETY: Safe as the fd is guaranteed to be valid here.
        let epoll_file = unsafe { File::from_raw_fd(epoll_out_fd) };
    ...
    /// Process a THREAD_SPECIFIC_BACKEND_EVENT received by VhostUserVsockBackend.
    ...
    /// Handle a THREAD_SPECIFIC_BACKEND_EVENT by either accepting a new connection or
...
impl VhostUserVsockThread for VhostUserVsockTxThread {
    ...
    fn set_vring_worker(
        ...
        self.vring_worker
            .as_ref()
            .unwrap()
            .register_listener(self.get_epoll_fd(), EventSet::IN, u64::from(THREAD_SPECIFIC_BACKEND_EVENT))
            .unwrap();
    }
...
pub(crate) struct VhostUserVsockRxThread {
    ...
    /// epoll fd to which new host connections are added to listen for EPOLLIN events.
    epoll_file: File,
    /// Instance of VringWorker.
    vring_worker: Option<Arc<VringEpollHandler<ArcVhostBknd, VringRwLock, ()>>>,
    /// VsockThreadBackend instance.
...
impl VhostUserVsockRxThread {
    pub fn new(epoll_in_fd: i32, epoll_out_fd: i32, guest_cid: u64, host_sock: UnixListener, thread_backend: Arc<VsockThreadBackend>) -> Result<Self> {
        let epoll_file = unsafe { File::from_raw_fd(epoll_in_fd) };
    ...
    /// Handle a THREAD_SPECIFIC_BACKEND_EVENT by either accepting a new connection or
    /// forwarding a request to the appropriate connection object. The `evset` parameter
    /// passed to the function must be `epoll::Events::EPOLLIN`. 
    fn handle_backend_event(&mut self, fd: RawFd, evset: epoll::Events) {
        if evset != epoll::Events::EPOLLIN {
            return;
        }
        ...
        if fd == self.host_sock {
            ...
            if !self.thread_backend.listener_map.read().unwrap().contains_key(&fd)
            {
                ...
                // established with the guest
                EpollHelpers::epoll_unregister(self.epoll_file.as_raw_fd(), fd).unwrap();
                EpollHelpers::epoll_unregister(self.epoll_out_fd, fd)
                    .unwrap_or_else(|err| {
                        warn!("Could not remove epoll listener for fd from the epoll_out_fd")
                    });
